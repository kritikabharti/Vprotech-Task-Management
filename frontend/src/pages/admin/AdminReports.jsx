...
const SUMMARY_COLUMNS = [
  ...
  { key: 'teamLead', label: 'Team Lead', render: (r) => r.teamLead?.fullName || '-' },
...
export default function AdminReports() {
  ...
  const handleExport = async (format) => {
    setExporting(true);
    try {
      const res = await axiosClient.get('/reports/export', { params: { type: tab, format, ...paramsForTab() }, responseType: 'blob' });
      const url = window.URL.createObjectURL(new Blob([res.data]));
      const link = document.createElement('a');
      link.href = url;
      link.download = `${tab}-company-report.${format === 'excel' ? 'xlsx' : 'pdf'}`;
  ...
  return (
    <div className="space-y-5">
      ...
      <div className="card p-4 flex flex-wrap items-end gap-3">
        {tab === 'daily' && <div><label className="label">Date</label><input type="date" max={todayISO()} className="input-field" value={date} onChange={(e) => setDate(e.target.value)} /></div>}
        {tab === 'weekly' && <div><label className="label">Any date in week</label><input type="date" max={todayISO()} className="input-field" value={weekStart} onChange={(e) => setWeekStart(e.target.value)} /></div>}
        {tab === 'monthly' && <div><label className="label">Month</label><input type="month" className="input-field" value={month} onChange={(e) => setMonth(e.target.value)} /></div>}
        {tab === 'custom' && (
          <>
