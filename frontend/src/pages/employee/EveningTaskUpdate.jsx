...
// Keeps completion % and status roughly in sync without fighting the user:
// picking "Completed" jumps % to 100, "Not Completed" to 0; typing a %
// manually is always respected as-is.
...
export default function EveningTaskUpdate() {
  ...
  // When the day's evening update is already submitted, default to showing
  // the success summary instead of the form - but editTasks lets the
  // employee flip back to the editable form at any time, since the backend
  // keeps accepting edits until the report is actually reviewed/approved.
  ...
  const loadDay = useCallback(async (d) => {
    ...
    try {
      ...
      // IMPORTANT: this sets loadError, NOT just a toast. `report` stays
      // null on failure same as it would for a genuine "no plan yet" day -
      // loadError is what lets the render below tell those two cases apart
      // instead of showing a misleading "go plan your day" message for an
      // actual network/auth/server failure.
  ...
  // Matches the backend exactly: evening_submitted is still editable right
  // up until the report is approved (or sent back for correction, which is
  // also editable). Only 'approved' - and the pre-plan 'draft' state, which
  // can't reach this branch anyway - are truly locked.
    ...
    <div className="max-w-4xl mx-auto space-y-5">
      <div className="card p-5">
        <div className="flex flex-wrap items-center justify-between gap-3 mb-1">
          <h2 className="text-lg font-semibold text-navy-800">Evening Task Update</h2>
          <input
            type="date"
            value={date}
            max={todayISO()}
      ...
      {loading ? (
        ...
        <div className="card">
          <EmptyState
            ...
            action={<Link to="/employee/morning-update" className="btn-primary">Go to Morning Update</Link>}
          />
        </div>
      ) : showSuccessSummary ? (
        <div className="card p-8">
          <div className="flex flex-col items-center justify-center text-center py-10">
            ...
            <p className="text-xs text-navy-400 mt-4">
              Forgot to plan something? <Link to="/employee/morning-update" className="text-navy-600 underline">Add it to your Morning Update</Link> first, then it'll appear here.
            </p>
          </div>
        </div>
      ) : locked ? (
