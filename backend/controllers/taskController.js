...
// /**
//  * Morning update cutoff: 9:40 AM server time.
//  *
//  * Mirrors the frontend's client-side lock (MorningTaskUpdate.jsx) so
//  * the deadline is actually enforced - a request made directly against
//  * the API (bypassing the UI) can't skip it.
//  */
// const MORNING_CUTOFF_HOURS = 9;
// const MORNING_CUTOFF_MINUTES = 40;

// function isPastMorningCutoff(now = new Date()) {
//   const cutoff = new Date(now);
//   cutoff.setHours(MORNING_CUTOFF_HOURS, MORNING_CUTOFF_MINUTES, 0, 0);
//   return now >= cutoff;
// }

// /**
//  * Whether `taskDate` (a calendar-normalized UTC-midnight Date, see
//  * toCalendarDate) refers to today, in server-local time.
//  */
// function isCalendarDateToday(taskDate, now = new Date()) {
...
//     // ------------------------------------------------------------
//     // 9:40 AM morning cutoff (employees only, today's date only)
//     // ------------------------------------------------------------
//     // Mirrors the client-side lock in MorningTaskUpdate.jsx, but
//     // enforced here so it can't be bypassed by calling the API
//     // directly. Team leads filing their own plan are unaffected -
//     // this only governs employees.

//     const now = new Date();

//     const isLateWindow =
//       employee.role === 'employee' &&
//       isCalendarDateToday(taskDate, now) &&
//       isPastMorningCutoff(now);
...
//         throw new ApiError(
//           400,
//           'Morning tasks are locked after 9:40 AM. Drafts can no longer be saved - submit with a reason instead.'
//         );
//       }
...
//         report.lateSubmissionReason = lateReason;
//         report.lateSubmittedAt = new Date();
//       }
//     }
  ...
  asyncHandler(
    async (req, res) => {
      ...
      if (isLateWindow) {
        ...
        /*
         * Draft saving after 9:40 remains disabled.
         *
        ...
        if (!submit) {
          throw new ApiError(
            400,
            'Morning tasks are locked after 9:40 AM. Submit the update with a reason instead.'
          );
        }
         ...
         * ------------------------------------------------------
         * CASE 1:
         * Already submitted late.
         *
         * Do NOT require another reason.
        ...
        if (wasAlreadyLate) {
          ...
          /*
           * If the old record has no reason for some reason,
           * accept a new one as recovery.
           */
          if (!lateReason) {
            lateReason =
              String(
          ...
          if (!lateReason) {
            throw new ApiError(
              400,
              'The original late submission reason could not be found. Please enter a reason.'
            );
          }
